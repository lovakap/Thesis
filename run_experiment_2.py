...
import numpy as np
import os
# run 2 - comparison of landscapes
noise_mean = 0.0
# noise_std = [0.1, 0.3, 0.5, 0.7, 0.9, 8]
noise_std = [0.5, 1., 2., 4., 6., 8.]
blob_mean = 0.
blob_std = 2.
image_size = 100
blob_size = 70
...
# exp_num = f'Image_Size_{image_size}_Ratio_{float("{:.3f}".format(blob_size / image_size))}_Constant_Blob_{1}/'
exp_num = f'Image_Size_{image_size}_Ratio_{float("{:.3f}".format(blob_size / image_size))}_Mean_{blob_mean}_std_{blob_std}/'
# exp_num = f'Clip_Ratio_{float("{:.3f}".format(blob_size / image_size))}_Experiment_{1}/'
...
for std in noise_std:
    ...
    # path = create_result_dir(f'{exp_num}_Noise_STD_{std}')


    single_object, center, snr = get_image_for_testing(image_size=image_size, sub_image_size=blob_size, add_noise=add_noise,
    ...
    # filter_size = [75, 151, 251, 351, 405, 500]
    filter_size = [0.1, 0.25, 0.5, 0.75, 1.]
    # filter_size = [151, 301, 501, 751, 1000]
    for f_size in [int(image_size*i) for i in filter_size]:
        ...
        save_triple_plot(single_object=filtered_single_object, empty=filtered_empty, glued=filtered_glued_object,
                         filter_size=f_size, path=path, snr=snr, max_val=max_val, mark_center=mark_center, x=center[1], y=center[0])

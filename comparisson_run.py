from Utils import get_image_for_testing, create_result_dir, save_plot, get_empty_image, get_glued_image, \
    ...
    save_double_power_spec, get_ps, save_double_radial_mean
...
# res_dir = 'comparison/'
# res_dir = 'hist_comparison/'
# res_dir = 'ps_comparison/'
...
for std in noise_std:
    ...
    empty_image2 = get_empty_image(image_size=image_size, noise_mean=noise_mean, noise_std=std, add_noise=add_noise)
    ...
    # # fr, ps = get_ps(single_object)
    # fr, ps = get_ps(empty_image2)
    # fr2, ps2 = get_ps(empty_image)
    ...
    for f_size in [int(image_size * i) for i in filter_size]:
        ...
        # save_double_plot_var(filtered_single_object, filtered_empty, filter_size=f_size, path=path, snr=snr,
        #                      max_val=max_val, mark_center=mark_center, x=center[1], y=center[0])
        save_double_radial_mean(filtered_single_object, filtered_empty, filter_size=f_size, path=path, snr=snr,
                             max_val=max_val, mark_center=mark_center)
